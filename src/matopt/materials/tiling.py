...
from matopt.materials.geometry import Parallelepiped, Cylinder, CylindricalSector
...
class LinearTiling(Tiling, ABC):
    ...
    @classmethod
    def fromLatticeVector(cls, lattice_vector):
        """Create tiling from single lattice vector.
        
        Args:
            lattice_vector (numpy.ndarray): 1D array representing periodicity direction
            
        Returns:
            LinearTiling: New tiling instance
        """
        instance = cls([lattice_vector])
        instance._TilingDirections = np.vstack([
            np.zeros(3),  # No periodicity in x-direction
            np.zeros(3),  # No periodicity in y-direction
            lattice_vector
        ])
        return instance
    
    @classmethod
    def fromLatticeVectors(cls, lattice_vectors):
        """Create tiling from lattice vectors."""
        instance = cls(lattice_vectors)
        instance._TilingDirections = np.vstack([
            np.zeros(3),  # No periodicity in x-direction
            np.zeros(3),  # No periodicity in y-direction
            lattice_vectors[0]
...
class PlanarTiling(Tiling):
    ...
    @classmethod
    def fromLatticeVectors(cls, lattice_vectors_2d):
        """Create tiling from 2D lattice vectors.
        
        Args:
            lattice_vectors_2d (numpy.ndarray): 2x3 array of in-plane lattice vectors
            
        Returns:
            PlanarTiling: New tiling instance
        """
        parallelepiped = Parallelepiped(lattice_vectors_2d[0], lattice_vectors_2d[1], np.cross(lattice_vectors_2d[0], lattice_vectors_2d[1]))
        return cls(parallelepiped)
...
class CubicTiling(Tiling):
    ...
    @classmethod
    def fromLatticeVectors(cls, lattice_vectors):
        """Create tiling from arbitrary lattice vectors.
        
        Args:
            lattice_vectors (numpy.ndarray): 3x3 array of lattice vectors
            
        Returns:
            CubicTiling: New tiling instance
        """
        # Create a Parallelepiped from the lattice vectors
        parallelepiped = Parallelepiped(lattice_vectors[0], lattice_vectors[1], lattice_vectors[2])
        return cls(parallelepiped)
